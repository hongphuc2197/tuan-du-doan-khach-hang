...
def predict_student_potential_customers():
    try:
        ...
        # Tạo bảng tổng hợp theo user_id (chỉ lấy 100 user đầu tiên để test)
        ...
        user_summary['MntWines'] = user_summary['total_spent'] * 0.3
        user_summary['MntFruits'] = user_summary['total_spent'] * 0.1
        user_summary['MntMeatProducts'] = user_summary['total_spent'] * 0.2
        user_summary['MntFishProducts'] = user_summary['total_spent'] * 0.15
        user_summary['MntSweetProducts'] = user_summary['total_spent'] * 0.1
        user_summary['MntGoldProds'] = user_summary['total_spent'] * 0.15
        
        # Các features khác
        user_summary['NumDealsPurchases'] = user_summary['purchase_events'] * 0.3
        user_summary['NumWebPurchases'] = user_summary['purchase_events'] * 0.5
        user_summary['NumCatalogPurchases'] = user_summary['purchase_events'] * 0.2
        user_summary['NumStorePurchases'] = user_summary['purchase_events'] * 0.3
        user_summary['NumWebVisitsMonth'] = user_summary['total_actions'] * 0.8
        ...
        # Lọc khách hàng tiềm năng (xác suất > 0.5) và chỉ lấy top 50
        potential_customers = user_summary[user_summary['probability'] > 0.5].copy()
        potential_customers = potential_customers.sort_values('probability', ascending=False).head(50)
