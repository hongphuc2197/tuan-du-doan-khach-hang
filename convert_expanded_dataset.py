...
def convert_expanded_dataset_to_marketing_campaign():
    """
    Chuyển đổi dữ liệu mở rộng (687 users) sang format tương thích với marketing_campaign.csv
    """
    print("Đang đọc file user_actions_expanded_687.csv...")
    
    # Đọc file CSV mở rộng
    df = pd.read_csv('user_actions_expanded_687.csv')
    
    print(f"Dữ liệu mở rộng có {len(df)} dòng và {len(df.columns)} cột")
    ...
    # Lưu file mới
    output_file = 'marketing_campaign_687_users.csv'
    ...
    print("\nThống kê chi tiêu:")
    print(f"Chi tiêu trung bình cho rượu vang: {converted_df['MntWines'].mean():.2f}")
    print(f"Chi tiêu trung bình cho thịt: {converted_df['MntMeatProducts'].mean():.2f}")
    print(f"Chi tiêu trung bình cho cá: {converted_df['MntFishProducts'].mean():.2f}")
    
    return converted_df

if __name__ == "__main__":
    converted_data = convert_expanded_dataset_to_marketing_campaign()
    print("\nChuyển đổi dữ liệu mở rộng hoàn tất!")