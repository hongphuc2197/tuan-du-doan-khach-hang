...
def convert_user_actions_to_marketing_campaign():
    """
    Chuyển đổi dữ liệu từ user_actions_with_profile_rows.csv sang format tương thích với marketing_campaign.csv
    """
    print("Đang đọc file user_actions_with_profile_rows.csv...")
    
    # Đọc file CSV
    df = pd.read_csv('user_actions_with_profile_rows.csv')
    
    print(f"Dữ liệu gốc có {len(df)} dòng và {len(df.columns)} cột")
    print("Các cột trong dữ liệu gốc:")
    ...
    print("\nThống kê income level:")
    # Tạo cột income_level từ dữ liệu gốc
    income_levels = []
    for user_id in unique_users:
        user_data = df[df['user_id'] == user_id].iloc[0]
        income_levels.append(user_data.get('income_level', 'Unknown'))
    
    converted_df['income_level'] = income_levels
    print(converted_df.groupby('income_level')['Income'].mean())
    
    return converted_df

if __name__ == "__main__":
    converted_data = convert_user_actions_to_marketing_campaign()
